...
def animate_light_style():
    ...
    dataframes = [pd.read_csv(f).iloc[::10].reset_index(
        drop=True) for f in file_list]
    max_frames = max(len(df) for df in dataframes)

    # Appearance Setup (Light Mode)
    ...
    for i in range(len(dataframes)):
        ...
        scat = ax.scatter([], [], [], color=colors[i], s=60,
